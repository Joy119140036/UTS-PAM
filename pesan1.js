...
import { JADWAL } from '../data/Data'; 

import React, { useState } from 'react';
import {StyleSheet, Text, SafeAreaView, TouchableOpacity, StatusBar, View, FlatList} from 'react-native';


const pesan1 = ({ navigation, route }) => {
    const data = route.params.text;
    const scheduleList = JADWAL.filter(item =>
        item.dermaga_keberangkatan === data.kb &&
        item.dermaga_tujuan === data.kd &&
        item.kelas_layanan === data.kl &&
        item.tanggal_masuk === data.tgl &&
        item.jam_masuk === data.wkt);
